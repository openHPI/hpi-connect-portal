...
class User < ActiveRecord::Base
  ...
  def self.update_alumni_data(row)
    matched_user = User.where("LOWER(alumni_email) LIKE ?", row[:alumnimail].gsub("@hpi-alumni.de", "").downcase).first if row[:alumnimail]
    ...
    if matched_user
      if row[:private_email]
        row[:emailverteiler].gsub!(row[:private_email], matched_user.email)
