class ApplicationsMailer < ActionMailer::Base
  ...
  def new_application_notification_email(application)
    if(!application.user.cv.path.nil?)
      attachments[application.user.cv_file_name] = File.read(application.user.cv.path)
    end
    send_mail_for_application_to_wimi(application, (t "applications_mailer.wimi.new_application.subject"))
  end
  ...
  private
    def send_mail_for_application_to_user(application, subject)
        ...
        def send_mail_for_application_to_wimi(application, subject)
        @application = application    
