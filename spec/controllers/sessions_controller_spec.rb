...
describe SessionsController do
  ...
  describe "POST create" do
    ...
    it "logs in user with correct email and password (case insensitive)" do
      post 'create', params: { session: {email: user.email.swapcase, password: 'password'} }
      expect(current_user).to eq(user)
    end
    ...
    it "redirects to root with incorrect email" do
      post 'create', params: { session: {email: 'some_test@email.com', password: 'password'} }
      expect(response).to redirect_to(root_path)
    end

    it "redirects to employers home if staff logs in" do
      post 'create', params: { session: {email: staff.email, password: "password123"} }
    ...
    it "redirects to user edit page if alumnus with invalid mail adress logs in" do
      ...
      post 'create', params: { session: {email: alumnus.email, password: 'password123'} }
