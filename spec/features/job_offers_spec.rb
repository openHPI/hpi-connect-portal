...
describe "the job-offers page" do
  ...
  before(:each) do
    ...
    login @student1.user
    @epic = FactoryBot.create(:employer, name:"EPIC", booked_package_id: 2)
    @active = JobStatus.active
    @pending = JobStatus.pending
    @test_employer = FactoryBot.create(:employer)
    @staff = FactoryBot.create(:staff)
    @job_offer_1 = FactoryBot.create(:job_offer, title: "TestJob1", employer: @test_employer, status: @active)
    @job_offer_2 = FactoryBot.create(:job_offer, title: "TestJob2", employer: @epic, status: @active)
    @job_offer_3 = FactoryBot.create(:job_offer, title: "TestJob3", state_id: 3, category_id: JobOffer::CATEGORIES.index("graduate_job"), employer: @epic, status: @active)
    @job_offer_4 = FactoryBot.create(:job_offer, title: "Pending Job", category_id: JobOffer::CATEGORIES.index("graduate_job"), employer: @epic, status: @pending)
  end
  ...
  it "should show prolonged Date if available" do
    JobOffer.delete_all
    job_offer = FactoryBot.create(:job_offer, status: JobStatus.active, prolonged: true, prolonged_at: Date.current,
...
describe "job_offers_history" do
  ...
  describe "Show Archive Job Offers" do
    before :each do
      ...
      @closed_job_offer_for_staff_1 = FactoryBot.create(:job_offer, status: JobStatus.closed, employer: staff1.employer)
      @closed_job_offer_for_staff_2 = FactoryBot.create(:job_offer, status: JobStatus.closed, employer: staff2.employer)
      login staff1.user
